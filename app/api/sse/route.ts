  ...
  const stream = new ReadableStream({
    async start(controller) {
      try {
        ...
        // Registrations are embedded in sessions — a single Change Stream covers everything.
        const sessionStream = Session.watch([], {
          fullDocument: 'updateLookup'
        });

        // Handle session changes (includes embedded registration changes)
        sessionStream.on('change', (change) => {
          if (!isActive) return;
          try {
            const ts = Date.now();
            const op = change.operationType;
            // Emit both event types so all frontend listeners are triggered
            controller.enqueue(encoder.encode(`data: ${JSON.stringify({ type: 'sessions', operation: op, timestamp: ts })}\n\n`));
            controller.enqueue(encoder.encode(`data: ${JSON.stringify({ type: 'registrations', operation: op, timestamp: ts })}\n\n`));
          } catch (error) {
